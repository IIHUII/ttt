...
if NotificationConfig.IsMobile and isMobile then
        return nil
    end

    if NotificationConfig.IsPC and isPC then
        return nil
    end
...
function OrionLib:MakeWindow(WindowConfig)
    ...
    local ToggleIcon = tostring(Configs.ToggleIcon or "rbxassetid://8834748103")
...
local T = Instance.new("ImageButton", Orion) 
local sizeValue = isPC and 80 or 60
T.Size = UDim2.new(0, sizeValue, 0, sizeValue)
local xPos = isPC and 0.18 or (isMobile and 0.10 or 0.18)
...
local function ToggleUI()
    ...
    if UIHidden and not isMobile then
        OrionLib:MakeNotification({
            Name = "UI Its Hidden",
            Content = "Press RightShift For Show UI Again!",
            Time = 5
        })
    end
...
AddConnection(CloseBtn.MouseButton1Up, ToggleUI)
T.MouseButton1Click:Connect(ToggleUI)
...
if TabConfig.IsMobile and isMobile then
        return nil
    end

    if TabConfig.IsPC and isPC then
        return nil
    end
    ...
    if ButtonConfig.IsMobile and isMobile then
        return nil
    end

    if ButtonConfig.IsPC and isPC then
        return nil
    end
...
if ToggleConfig.IsMobile and isMobile then
        return nil
    end

    if ToggleConfig.IsPC and isPC then
        return nil
    end
...
if SliderConfig.IsMobile and isMobile then
        return nil
    end

    if SliderConfig.IsPC and isPC then
        return nil
    end
...
if DropdownConfig.IsMobile and isMobile then
        return nil
    end

    if DropdownConfig.IsPC and isPC then
        return nil
    end
...
if BindConfig.IsMobile and isMobile then
        return nil
    end

    if BindConfig.IsPC and isPC then
        return nil
    end
...
if TextboxConfig.IsMobile and isMobile then
        return nil
    end

    if TextboxConfig.IsPC and isPC then
        return nil
    end
...
if ColorpickerConfig.IsMobile and isMobile then
        return nil
    end

    if ColorpickerConfig.IsPC and isPC then
        return nil
    end
...
if SectionConfig.IsMobile and isMobile then
        return nil
    end

    if SectionConfig.IsPC and isPC then
        return nil
    end
